...
if(!args[0]) return message.channel.send(nn.setColor('#00001').setTitle('Bir hata oldu!').setDescription(`reklam kısıtmak istersen **a!reklam kısıt** yazmalısın.`))
if(args[0] === 'kısıt') {
data.set(`reklam.${message.guild.id}`, true);
return message.channel.send(nn.setTitle(`Hazır!`).setDescription('Reklam kısıtlaması başarıyla aktifleştirildi.')).then(a => a.delete({timeout: 10000}));
} else if(args[0] === 'kapat') {
data.delete(`reklam.${message.guild.id}`);
return message.channel.send(nn.setTitle('Hazır!').setDescription('Reklam kısıtlaması başarıyla kapatıldı.')).then(a => a.delete({timeout: 10000}));
}
};
exports.conf = {
  enabled: true,
