...
if(!args[0]) return message.channel.send(nn.setColor('#00001').setTitle('Bir hata oldu!').setDescription(`küfür kısıtmak istersen **a!küfür kısıt** yazmalısın.`))
if(args[0] === 'kısıt') {
data.set(`küfür.${message.guild.id}`, true);
return message.channel.send(nn.setTitle(`Hazır!`).setDescription('Küfür kısıtlaması başarıyla aktifleştirildi!')).then(a => a.delete({timeout: 10000}));
} else if(args[0] === 'kapat') {
data.delete(`küfür.${message.guild.id}`);
return message.channel.send(nn.setTitle('Hazır!').setDescription('Küfür kısıtlaması başarıyla kapatıldı.')).then(a => a.delete({timeout: 10000}));
}
};
exports.conf = {
  enabled: true,
