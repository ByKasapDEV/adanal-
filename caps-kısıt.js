...
return message.channel.send(nn.setTitle(`Hazır!`).setDescription('Büyük harf kısıtlaması başarıyla aktifleştirildi!')).then(a => a.delete({timeout: 10000}));
} else if(args[0] === 'kapat') {
data.delete(`caps.${message.guild.id}`);
return message.channel.send(nn.setTitle('Hazır!').setDescription('Büyük harf kısıtlaması başarıyla kapatıldı.')).then(a => a.delete({timeout: 10000}));
}
};
exports.conf = {
  enabled: true,
