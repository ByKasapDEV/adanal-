...
if(!args[0]) return message.channel.send(new Discord.MessageEmbed().setTitle('Bir hata oluştu!').setDescription(`${message.author} İçerik için yazı yazmayı unuttunuz.`));
message.delete();
return message.channel.send(new Discord.MessageEmbed() .setTitle(`${message.guild.name} Sunucusunda Duyuru Yapıldı!`) .setDescription(args[0]));
