...
if(!args[0]) return message.channel.send(nn.setColor('#00001').setTitle('Bir hata oldu!').setDescription(`Selam açmak istersen **a!selam aç** yazmalısın.`))
if(args[0] === 'aç') {
data.set(`selams.${message.guild.id}`, true);
return message.channel.send(nn.setTitle(`Hazır!`).setDescription('Selam mesajı başarıyla aktifleştirildi.')).then(a => a.delete({timeout: 10000}));
} else if(args[0] === 'kapat') {
data.delete(`selams.${message.guild.id}`);
return message.channel.send(nn.setTitle('Hazır!').setDescription('Selam mesajı başarıyla kapatıldı.')).then(a => a.delete({timeout: 10000}));
}
};
exports.conf = {
  enabled: true,
